      ...
      <div v-if="page.moreProps">
        ...
        <van-popup v-model="page.rTeaShow" round position="bottom" :style="{ height: '70%' }">
          <van-list>
            ...
            >
              <van-tag round type="danger">{{ item.r_tea_total||0 }}</van-tag>
  ...
  methods: {
    ...
    async loadArmrestData(page) {
      ...
      if (this.armrestList.length === 0) {
        const armrest = await fetchData(this, "armrest/api/fetch", {});
    ...
    async loadSitData(page) {
      ...
      if (this.sitList.length === 0) {
        const sit = await fetchData(this, "sit_board/api/fetch", {});
    ...
    async loadSTea(page) {
      ...
      if (this.s_teaList.length === 0) {
        const stea = await fetchData(this, "s_tea_table/api/fetch", {});
    ...
    async loadRTea(page) {
      ...
      if (this.r_teaList.length === 0) {
        const rtea = await fetchData(this, "r_tea_table/api/fetch", {});
    ...
    async loadPillowData(page) {
      ...
      if (this.pillowList.length === 0) {
        const rtea = await fetchData(this, "pillow_stock/api/fetch", {});
    ...
    async loadPillow_color(page) {
      ...
      if (this.pillow_colorList.length === 0) {
        const rtea = await fetchData(this, "pillow_color/api/fetch", {});
    ...
    async loadPillow_speci(page) {
      ...
      if (this.pillow_speciList.length === 0) {
        const rtea = await fetchData(this, "pillow_speci/api/fetch", {});
    ...
    async loadWaist(page) {
      ...
      if (this.waistList.length === 0) {
        const rtea = await fetchData(this, "waist/api/fetch", {});
    ...
    getSitBoard(page, Id) {
      ...
      const { sit_name_cn, sit_size_name_cn } = obj;
      page.sit_name_cn = sit_name_cn;
      page.sit_size_name_cn = sit_size_name_cn;
    ...
    getPillow_color(page, Id) {
      ...
      const { pillow_color_name_cn, pillow_name_cn } = obj;
      page.pillow_color_name_cn = pillow_color_name_cn;
      page.pillow_name_cn = pillow_name_cn;
      this.onClosePop(page);
    },
    ...
    getWaist(page, Id) {
      ...
      const { waist_color_name_cn, waist_name_cn } = obj;
      page.waist_color_name_cn = waist_color_name_cn;
      page.waist_name_cn = waist_name_cn;
      this.onClosePop(page);
    },
    ...
    async setChaise(page) {
      ...
      if (page.chaise_checked) {
        ...
        const chaise = await fetchData(this, "chaise/api/fetch", { Id });
        if (chaise.data[0].chaise_total !== "0") {
          this.$set(page, "chaise_name_cn", chaise.data[0].chaise_name_cn);
    ...
    onSubmit() {
      ...
      this.pages.forEach(el => {
        const obj = {};
        if (el.sofa_totalPrice === 0 && el.corner_totalPrice === 0) {
          // 提示
          Notify({ type: "warning", message: "请填写单价和数量" });
          return;
        } else {
          if (el.sofa_totalPrice !== 0) {
            obj.sofa_totalPrice = el.sofa_totalPrice;
            obj.sofa_singlePrice = el.sofa_singlePrice;
            obj.sofa_number = el.sofa_number;
          }
          if (el.corner_totalPrice !== 0) {
            obj.corner_totalPrice = el.corner_totalPrice;
            obj.corner_singlePrice = el.corner_singlePrice;
    ...
    async postOrderData(data) {
      const res = await postData(this, "outorder/api/create", data);
    },
    addMore() {
